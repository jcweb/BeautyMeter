        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (attr) {
                ...
                case R.styleable.circleProgressBar_circleWidth:
                    ...
                    break;
                case R.styleable.circleProgressBar_tip:
                    CharSequence charSequence = ta.getText(attr);
                    if (charSequence != null) {
                        strTip = charSequence.toString();
                    }
                    break;
                case R.styleable.circleProgressBar_up:
                    charSequence = ta.getText(attr);
                    if (charSequence != null) {
                        topTip = charSequence.toString();
                    }
                    break;
                case R.styleable.circleProgressBar_progress_size:
                    progressSize = ta.getDimensionPixelSize(attr, (int) TypedValue.applyDimension(
                            TypedValue.COMPLEX_UNIT_DIP, 60, getResources().getDisplayMetrics()));
                    break;
                case R.styleable.circleProgressBar_tip_size:
                    tipSize = ta.getDimensionPixelSize(attr, (int) TypedValue.applyDimension(
                            TypedValue.COMPLEX_UNIT_DIP, 20, getResources().getDisplayMetrics()));
                    break;
                case R.styleable.circleProgressBar_time:
                    duration = ta.getInt(attr,duration);
                    break;
                default:
                    break;
            }
...
//        circlePaint.setShadowLayer(10, 10, 10, Color.RED);
    ...
    private void drawText(Canvas canvas, int center, int radius) {
        String percent = context.getString(R.string.no_data);
        ...
        textPaint.setTextSize(progressSize); // 设置要绘制的文字大小
        ...
        textPaint.setTextAlign(Paint.Align.CENTER); // 设置文字居中，文字的x坐标要注意
        textPaint.setFakeBoldText(true);//加粗
        textPaint.setTextSize(tipSize); // 设置要绘制的文字大小
        textPaint.setStrokeWidth(0); // 注意此处一定要重新设置宽度为0,否则绘制的文字会重叠
        bounds = new Rect(); // 文字边框
        textPaint.getTextBounds(strTip, 0, strTip.length(), bounds); // 获得绘制文字的边界矩形
        fontMetrics = textPaint.getFontMetricsInt(); // 获取绘制Text时的四条线
        baseline = center + (fontMetrics.bottom - fontMetrics.top) / 2 - fontMetrics.bottom;
        canvas.drawText(strTip, center, baseline + (progressSize + tipSize * 2) / 2, textPaint); // 绘制表示进度的文字
        ...
        textPaint.setStrokeWidth(0); // 注意此处一定要重新设置宽度为0,否则绘制的文字会重叠
        bounds = new Rect(); // 文字边框
        textPaint.getTextBounds(topTip, 0, topTip.length(), bounds); // 获得绘制文字的边界矩形
        fontMetrics = textPaint.getFontMetricsInt(); // 获取绘制Text时的四条线
        baseline = center + (fontMetrics.bottom - fontMetrics.top) / 2 - fontMetrics.bottom;
        canvas.drawText(topTip, center, baseline - (progressSize + tipSize * 2) / 2, textPaint); // 绘制表示进度的文字
    }
...
//            animator.setInterpolator(new OvershootInterpolator());//动画会有回弹的效果
